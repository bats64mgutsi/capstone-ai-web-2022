        ...
        <div class="w-4/12 mx-auto py-6 px-6 lg:px-8">
            ...
            <form @submit.prevent="login" class="mt-8 space-y-6" action="#">
                <div>
                    ...
                    <input type="text" v-model="loginCreds.username" name="username" id="username" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="admin" required>
                </div>
                <div>
...
const loginCreds = reactive({ username: "", password: "" });
...
const login = async () => {
    try {
        ...
        // goToPage('authors');
        router.push({ path: '/admin/authors' });
    } catch (e) {
        handleError(e);
    }
};
