<template>  
    <div class="w-11/12 mt-2 mx-auto overflow-x-auto relative shadow-md sm:rounded-lg mb-6">
        <div class="flex flex-row justify-between items-center mb-3">
            ...
            <UploadNrfListDialog v-if="isLoggedIn()" />
        </div>
...
import { handleError, refreshData, isLoggedIn } from "../util";
...
const pageContent = ref<Array<Author>>([]);
...
const setSearchedContent = () => {
    searchedContent.value = authors.value.length ? JSON.parse(JSON.stringify(authors.value)) : [];
}
...
const getValidEndIndex = (content: Array<Author>, endIndex: number) => {
...
const getContentWithFiltersApplied = (formattedContent: Array<Author>, filters: Array<string>) => {
    return formattedContent.filter(row => containsFilters(row, filters));
};


const containsFilters = (row: Author, filters: Array<string>) => {
